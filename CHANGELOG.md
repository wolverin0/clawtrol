...
- drop nonexistent status column from TaskRun.create + integrity expectation (`5aadacb`)
...
- update CHANGELOG.md [skip ci] (`06248e7`)
...
- **ci:** factory_cron_sync ENOENT rescue, invite_code remove stale attrs, swarm N+1, board/swarm system tests (`cdfbdb3`)
- **ci:** remove tests for non-existent session attributes, fix zerobitch connect order (`75ea584`)
- **ci:** disable Bullet unused_eager_loading in CI, fix remaining rubocop lint in services/migrate (`a2c9a79`)
...
- update CHANGELOG.md [skip ci] (`753d14a`)
