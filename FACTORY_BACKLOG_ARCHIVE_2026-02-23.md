...
## 🚨 CLAWTROL 2026-02-23 TOP PRIORITY (Execute First)

- [ ] **Mission Control Health Dashboard**: single panel for route health, queue health, provider health (`memory_search`), and service status with OK/WARN/FAIL.
- [ ] **Unified Navbar Registry**: one canonical source for desktop + mobile nav to prevent drift/missing tabs.
- [ ] **Dead/Empty Route Scanner**: internal scan that flags 500s, empty pages, `content missing`, turbo-frame mismatches, and JS console errors.
- [ ] **Error Inbox**: aggregate Rails/Turbo/frontend errors into one inbox with severity and one-click task creation.
- [ ] **Roadmap Executor Sync**: sync markdown checkboxes in `docs/roadmaps/*` with board tasks and evidence links.
- [ ] **Burst Mode Control**: explicit toggle for `regular_tasks` vs `swarm_factory_zeroclaw`, with auto-expire and audit log.
- [ ] **Factory Promotion Gate**: block promotion unless test/lint/e2e checks pass and diff summary is generated.
- [ ] **Cross-Model Review Pipeline**: implementer model + reviewer model + validator model handoff with final consolidated report.
- [ ] **Learning Inbox Wiring**: connect self-audit/cognitive outputs to actionable learning proposals and task suggestions.
- [ ] **Cognitive Core Sync**: automatic sync of `mind/PROJECTS.md`, `mind/DECISIONS.md`, `mind/ERRORS.md`, and daily logs.
- [ ] **Runbook Generator**: generate incident runbooks from repeated failures with owner/ETA fields.
- [ ] **Daily Executive Digest**: send concise daily summary to Telegram (done, failed, blocked, next 3 steps).
