...
export default class extends Controller {
  ...
  renderCalendar() {
    ...
    for (let i = startDay - 1; i >= 0; i--) {
      ...
      html += `<button type="button" data-action="click->datepicker#selectDate" data-day="${day}" data-month="${month - 1}" data-year="${year}" class="h-8 w-8 rounded-md text-stone-300 dark:text-stone-600 hover:bg-stone-100 dark:hover:bg-white/10 transition-colors text-xs">${day}</button>`
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (isSelected) {
        ...
        classes += 'text-purple-600 dark:text-purple-400 font-semibold hover:bg-stone-100 dark:hover:bg-white/10'
      } else {
        classes += 'text-stone-600 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-white/10'
      }
