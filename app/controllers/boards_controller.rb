...
class BoardsController < ApplicationController
  ...
  def generate_persona
    result = PersonaGeneratorService.new(board: @board, user: current_user).call
      ...
      respond_to do |format|
        format.turbo_stream do
          render turbo_stream: turbo_stream.replace(
            ...
            locals: { persona: result.persona, board: @board }
          )
        end
        format.html { redirect_to board_path(@board), notice: "🤖 Agent persona generated: #{result.persona.name}" }
      end
    else
      redirect_to board_path(@board), alert: "Failed to generate persona: #{result.error}"
    end
  end
